...
- [x] T0.1 Create GitHub repo (Hree), add deploy key, branch protection on `main`
  - files: (GitHub settings)
- [x] T0.2 Monorepo scaffold: `apps/mobile` (move prototype), `apps/functions`, `packages/shared`, `infra`, `docs`
  - files: repo root `package.json` (workspaces), `apps/mobile/*` (moved)
- [x] T0.3 CI workflow: lint + typecheck + expo-doctor + rules tests + web-export smoke on every PR
  - files: `.github/workflows/ci.yml`
- [x] T0.4 EAS project link + `eas.json` (development/preview/production), `runtimeVersion: fingerprint`
  - files: `apps/mobile/eas.json`, `apps/mobile/app.json`
- [x] T0.5 Move the 12-file docs OS into `docs/`
  - files: `docs/*.md`
...
- [x] T1.2 Install RNFirebase v26 (pinned) + `expo-dev-client`; `newArchEnabled: true`; config plugins; googleServices files (gitignored)
  - files: `apps/mobile/package.json`, `apps/mobile/app.json`, `.gitignore`
- [x] T1.3 Google sign-in via `react-native-nitro-google-signin` → `signInWithCredential`; create `users/{uid}` on first login
  - files: `apps/mobile/src/data/auth.ts`, `apps/mobile/src/app/welcome.tsx`
- [x] T1.4 Apple sign-in via `expo-apple-authentication` (iOS); equal prominence, system button
  - files: `apps/mobile/src/data/auth.ts`, `apps/mobile/src/app/welcome.tsx`
- [x] T1.5 Signed-out routing + sign-out (clears cache + FCM token)
  - files: `apps/mobile/src/app/_layout.tsx`
- [x] T1.6 `firestore.rules` (default-deny, owner-only) + emulator tests in CI
  - files: `infra/firestore.rules`, `infra/firestore.rules.test.ts`

## T2 — Consent onboarding (Phase 1 · M2)

- [x] T2.1 Screen 1: notice + affirmative consent button + privacy policy link
  - files: `apps/mobile/src/app/onboarding/notice.tsx`
- [x] T2.2 Screen 2: default-off channel toggles (SMS/push in V1) with purpose lines
  - files: `apps/mobile/src/app/onboarding/channels.tsx`
- [x] T2.3 Screen 3: 18+ age gate
  - files: `apps/mobile/src/app/onboarding/age.tsx`
- [x] T2.4 Consent event logging → `users/{uid}/consentLog`; Settings → Privacy withdrawal
  - files: `apps/mobile/src/data/consent.ts`, `apps/mobile/src/app/(tabs)/settings.tsx`

## T3 — Transactions core (Phase 1 · M2)

- [x] T3.1 Repository layer: `TransactionRepository` (list/watch/add/update-category), `PlanRepository`, `BudgetRepository`, `DigestRepository`
  - files: `apps/mobile/src/data/*.ts`
- [x] T3.2 `packages/shared`: category taxonomy (frozen enum), merchant normalization, types
  - files: `packages/shared/taxonomy.ts`, `packages/shared/normalize.ts`, `packages/shared/types.ts`
- [x] T3.3 Add-transaction form (`react-hook-form` + `zod`): amount, merchant, category chips w/ glyphs, method, datetime, note
  - files: `apps/mobile/src/components/AddTransactionForm.tsx`, `apps/mobile/src/app/add.tsx`
- [x] T3.4 Home on live data: `dailyTotals` hero, search, tappable rows → detail sheet, recategorize
  - files: `apps/mobile/src/app/(tabs)/index.tsx`, `src/components/TransactionRow.tsx`, `src/components/TransactionSheet.tsx`
- [x] T3.5 Morning planner persisted to `plans/{yyyy-MM-dd}` (replaces component state)
  - files: `apps/mobile/src/components/MorningPlanner.tsx`, `apps/mobile/src/data/plans.ts`
- [x] T3.6 Insights/Budgets/Bills/History on live data, paginated (≤50), rollup reads only
...
- [x] T4.1 `onTransactionWrite` trigger: recompute `dailyTotals/{date}` + `budgets/{cat}.spentPaise` (idempotent, `increment()`)
  - files: `apps/functions/src/onTransactionWrite.ts` (+ jest tests)
- [x] T4.2 `scheduledDigest`: every 15 min, Asia/Kolkata; aggregate → `digests/{date}` → FCM
  - files: `apps/functions/src/scheduledDigest.ts` (+ jest tests)
- [x] T4.3 FCM lifecycle: permission → token → `fcmTokens` map → refresh → logout cleanup; background handler at module init; tap routing via `msg.data`
  - files: `apps/mobile/src/data/messaging.ts`, `apps/mobile/src/app/_layout.tsx`
- [x] T4.4 Evening wrap-up card renders `digests/{today}`; planned-vs-actual math
...
- [x] T5.1 `deleteAccount` Function + in-app flow + web deletion page
  - files: `apps/functions/src/deleteAccount.ts`, `apps/mobile/src/app/(tabs)/settings.tsx`
- [x] T5.2 `cleanupTokens` scheduled function
